...
function SmartContracts() {
    ...
    try {
      if (chainId == rinkebyChainId) {
        ...
        toast({
          ...
          description:
            "Not on a supported network. Please switch to Rinkbey or Ethereum Main Network",
          status: "error",
          duration: 9000,
          isClosable: true,
        });
      ...
      if (currentFunctions["type"] == "function") {
        if (currentFunctions["stateMutability"] == "view") {
          setReadFunctions((readFunctions) =>
            readFunctions.concat(JSON.stringify(currentFunctions))
          );
        } else {
          if (currentFunctions["payable"]) {
            currentFunctions["inputs"] = [
              { name: currentFunctions["name"], type: "uint256" },
            ];
          }
          setWriteFunctions((writeFunctions) =>
            writeFunctions.concat(JSON.stringify(currentFunctions))
          );
        }
      }
    }
  ...
  const handleClick = async (event) => {
    ...
    abiTextArea.value = JSON.stringify(JSON.parse(abiResponse), null, 2);
  ...
  if (!currentAccount) {
    ...
    return (
      <div className={styles.container}>
        <Layout route={router.pathname} />
        <Heading align="center">Explore your Contract</Heading>
        <form className="flex flex-col" onSubmit={handleClick}>
          <Box mt={8}>
            <InputGroup>
              <InputLeftAddon>Contact Address</InputLeftAddon>
              <Input
                placeholder="Enter the address of smart contract here"
                id="contract_address"
              />
            </InputGroup>
          </Box>
        ...
        >
          <Box mt={2} color="gray.500">
            <InputGroup>
              <Textarea id="abi_text_area"></Textarea>
            </InputGroup>
          </Box>
                        ...
                        <AccordionPanel pb={4}>
                          <form>
                            <Box mt={8}>
                              <InputGroup>
                                <Input
                                  placeholder={JSON.parse(functions).name}
                                  id={JSON.parse(functions).name}
                                />
                              </InputGroup>
                            </Box>
                            <Box align="right" mt={2}>
